...
def main():
    ...
    # MLflow is initialized with client = MlflowClient(), which creates an MLflow Tracking Server client that manages our experiment runs.
    # An MLflow experiment can comprise multiple ML training runs, so we start by creating and setting our first experiment (automl-insurance).
    # The logs are recorded locally into a /mlruns subfolder by default.
    client = MlflowClient()
    ...
    main_frame = h2o.import_file(path='ml_models/health_insurance/data/processed/train.csv')
    ...
    # Setup and wrap AutoML training with MLflow so that the model is logged inside an active MLflow run.
    with mlflow.start_run():
        ...
        # lb_path = f'ml_models/health_insurance/mlruns/{exp_id}/{run_id}/artifacts/model/leaderboard.csv'
