...
if st.button("Style Transfer"):
    if image is not None and style is not None:
        ...
        res = requests.post(f"http://finpy-api:8080/ml_models/style_transfer/{style}", files=files)
        img_path = res.json()
        image = Image.open(img_path.get("name"))
        st.image(image, width=500)


if st.button("Style Transfer Async"):
    if image is not None and style is not None:
        files = {"file": image.getvalue()}
        res = requests.post(f"http://finpy-api:8080ml_models/style_transfer/{style}", files=files)
        img_path = res.json()
        image = Image.open(img_path.get("name"))
        st.image(image)
